...
singleEval <- function(singleGene, X, y, type, criterion, criFun, family){ 
  if(length(which(singleGene != 0)) == 0){
    print("empty model")
    formula <- as.formula("y ~ 1")
  }
  else{
    formula <- as.formula(paste("y ~", paste(names(X[,which(singleGene != 0)]), collapse = " + ")))
  }
  print(formula)
  if(type == "lm"){
    #fit <- lm(y~X[,which(singleGene != 0)])
    #print(paste("y ~", paste(names(X[,which(singleGene != 0)]), collapse = " + ")))

    fit = lm(formula, data = cbind(y, X[,which(singleGene != 0)]))
  }
  if(type == "glm"){
    #fit <- glm(as.vector(y)~X[,which(singleGene != 0)], family)
    #formula <- as.formula(paste("y ~", paste("1 ", names(X[,which(singleGene != 0)]), collapse = " + ")))
    fit = glm(formula, data = cbind(y, X[,which(singleGene != 0)]), family)
  }
...
evalFunction <- function(X, y, currentGenePool, popSize, type = "lm", family = "gaussian", criterion = "AIC", criFun = NULL){
  ...
  result <- foreach(i = 1:popSize, .combine = c)  %do% {
